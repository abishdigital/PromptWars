...
const Navbar = () => {
  const { user, isAuthenticated, isCaregiver, logout } = useAuth();
  ...
  if (isCaregiver) {
    navLinks.push({ name: 'Caregiver Portal', path: '/caregiver', icon: Users });
  }
  ...
  return (
    <nav className="sticky top-0 z-40 w-full glass-card border-b border-slate-800/80">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to={isAuthenticated ? '/dashboard' : '/'} className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-lg tracking-tight text-white">
              Aura<span className="text-brand-400">Recovery</span>
            </span>
          </Link>
            ...
            {isAuthenticated ? (
              <div className="flex items-center space-x-2 pl-2 border-l border-slate-700">
                ...
                <button
                  ...
                  className="p-2 rounded-xl bg-slate-800/80 hover:bg-rose-950/40 text-slate-300 hover:text-rose-400 transition-colors border border-slate-700"
                  title="Logout"
                >
          ...
          {isAuthenticated ? (
            <>
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-base font-medium text-rose-400 hover:bg-rose-950/30"
              >
            ...
            <div className="space-y-2 pt-2">
              <Link
                to="/login"
                onClick={() => setMobileMenuOpen(false)}
                className="block w-full text-center px-4 py-2.5 rounded-xl text-slate-200 bg-slate-800"
              >
                Log In
              </Link>
              <Link
